...
export default function UserLoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // TODO: salvezi token, redirect etc.
    } catch (err) {
      console.error(err);
      alert("Network error");
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100 flex items-center justify-center">
      <div className="w-full max-w-6xl flex flex-col md:flex-row items-center justify-between gap-12 px-6 py-16">
        ...
        <div className="flex-1 text-center md:text-left space-y-5">
          <p className="tracking-widest text-xs text-sky-400 uppercase">Travel Platform</p>
          ...
          <p className="text-xs text-slate-600">Secure login • No extra fees • Instant access</p>
        </div>
        ...
        <Card className="flex-1 border-slate-800 bg-slate-950/70 backdrop-blur-sm shadow-lg">
          <CardHeader className="text-center">
            ...
            <p className="text-sm text-slate-400">Access your tickets, active routes and travel assistant.</p>
          </CardHeader>
